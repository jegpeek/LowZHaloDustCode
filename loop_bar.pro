...
notdone = 10 - done
  ;Information
percent_done = float(i-imin)/float(imax)
if (percent_done eq 0.0) then begin
  info = '0%'
  goto,first
endif
...
est_tot_time = elapsed_time/percent_done
remain_time = est_tot_time - elapsed_time
  ;Turn percent done into a integer
...
info = strtrim(string(percent_done),2)+'%, Approximate time remaining: '+time_left

first: ;Skip all the "time remaining" stuff if percent_done = 0.

; I n   L o o p 
;Very Beginning
if (i eq imin) then print,' Start time: '+systime(0)
;Beginning
...
;End
if ((notdone eq 0) and (i ne imax)) then $
  print,format='(%" [----------] %s \r",$)',info
;Very End
if (i eq imax) then begin
  common times,start_time
  end_time = systime(/julian)
  total_time = end_time - start_time - 0.5 ; - 0.5 = formatting weirdness
