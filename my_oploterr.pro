PRO my_oploterr, x, y, xerr, yerr, NOHAT=hat, HATLENGTH=hln, ERRTHICK=eth, $
      ERRSTYLE=est, THICK = thick, NOCLIP=noclip, ERRCOLOR = ecol, $
       _EXTRA = pkey, ANONYMOUS_ = Dummy_,MINY=miny
;+
; NAME:
;	OPLOTERR
; PURPOSE:
...
;; BRICE     oplot, xx, yy, NOCLIP=noclip,THICK = thick,_EXTRA = pkey 
 endif else begin 
...
FOR i = 0L, (n-1) DO BEGIN
    ...
    if ( miny ne 0 ) then begin
        if ( ylo(i) lt miny ) then begin
            if (yy(i) lt miny) then begin
                plots, [xx(i),xx(i)], [miny,yhi(i)], LINESTYLE=est,THICK=eth,  $
                  NOCLIP = noclip, COLOR = ecol
            endif else begin
                plots, [xx(i),xx(i)], [yy(i),yhi(i)], LINESTYLE=est,THICK=eth,  $
                  NOCLIP = noclip, COLOR = ecol
                plots, [xx(i),xx(i)], [yy(i),miny], LINESTYLE=est,THICK=eth,  $
                  NOCLIP = noclip, COLOR = ecol
            endelse

;            low_y_arrow_i=yy(i-2)   
;;            low_y_arrow_i=yy(i-2)-(yhi(i)-yy(i))/2.


;            plots, .9*xx(i), 1.2*low_y_arrow_i, LINESTYLE=est,THICK=eth,  $
;              NOCLIP = noclip, COLOR = ecol,/continue
;            plots, xx(i),low_y_arrow_i, LINESTYLE=est,THICK=eth,  $
;              NOCLIP = noclip, COLOR = ecol
;            plots, 1.1*xx(i), 1.2*low_y_arrow_i, LINESTYLE=est,THICK=eth,  $
;              NOCLIP = noclip, COLOR = ecol,/continue
        endif else begin
    ...
    if ( miny eq 0 ) then plots, [xx(i),xx(i)], [ylo(i),yhi(i)], LINESTYLE=est,THICK=eth,  $
		NOCLIP = noclip, COLOR = ecol
;                                                         Plot X-error bars 
;
